      ...
      <template v-else>
        <li class="layui-nav-item" @mouseover="show()" @mouseleave="hide()">
          ...
          <dl class="layui-nav-child layui-anim layui-anim-upbit" :class="{'layui-show':isHover}">
            ...
            <dd><router-link :to="{name: 'user-msg'}"><i class="iconfont icon-tongzhi" style="top: 4px;"></i>我的消息</router-link></dd>
            <dd><router-link :to="{name: 'user'}"><i class="layui-icon" style="margin-left: 2px; font-size: 22px;">&#xe68e;</i>我的主页</router-link></dd>
            <hr style="margin: 5px 0;">
            <dd><a class="text-center" @click="logout">退出</a></dd>
          </dl>
        </li>
      </template>
    </ul>
...
export default {
  ...
  methods: {
    show () {
      clearTimeout(this.hoverContrl)
      this.hoverContrl = setTimeout(() => {
        this.isHover = true
      }, 200)
    },
    hide () {
